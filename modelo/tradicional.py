...
def gaussianNB_parameters():
    return {
        'var_smoothing': [1e-9, 1e-8, 1e-0]
    }
...
def main():
    ...
    for data_set_file in os.listdir(data_set_folder):
        data_set_path = data_set_folder+"/"+data_set_file
        print('----------------------------------------------------------------------')
        print('fit ', data_set_path)
        fit_image(data_set_path, grid, data_test, label_test)

    print('----------------------------------------------------------------------')
    print('train finished')
    train_time = time.time() - star_train_time
    print_metrics(grid.best_estimator_, data_test, label_test, train_time)
